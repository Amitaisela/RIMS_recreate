    ...
    def simulation(self, env: simpy.Environment, writer, type, syn=False):
        ...
        while trans is not None:
            ...
            if trans.label is not None:
                ...
                # Ensure request_resource is assigned for all cases, including DSIM
                if type == 'rims_plus' or type == 'rims' or type == 'DSIM':
                    request_resource = resource.request()
                else:
                    request_resource = None  # Handle other types or unknown cases if needed

                if type == 'rims_plus':
                    queue = len(resource.queue[-1]) if len(resource.queue) > 0 else 0
                else:
                    queue = -1

                waiting = self.process.get_predict_waiting(str(self.id), pr_wip_wait, transition, rp_oc,
                                                           self.start_time + timedelta(seconds=env.now), queue=-1)

                if self.see_activity:
                    yield env.timeout(waiting)

                if request_resource is not None:
                    yield request_resource  # Now properly assigned for DSIM and other types
                ...
                rp_oc = self.process.get_occupations_resource(resource.get_name())
                initial_ac_wip = self.params.AC_WIP_INITIAL[
                    trans.label] if trans.label in self.params.AC_WIP_INITIAL else 0
                ac_wip = initial_ac_wip + resource_task.count

                # For DDPS, use predefined processing time
                duration = self.process.get_predict_processing(str(self.id), pr_wip_wait, transition, ac_wip, rp_oc,
                                                               self.start_time + timedelta(seconds=env.now), queue=-1)
