...
def main(argv):
    ...
    if LEN > 4:
        EMD_CI_lower, EMD_CI_higher = confidence_interval(
            list(EMD_normalize.values()))
    ...
    with open("results.csv", "a") as f:
        writer = csv.writer(f)
        if f.tell() == 0:
            writer.writerow(["Dataset", "Iteration", "Type", "MAE", "CI_Lower_MAE",
                            "CI_Upper_MAE", "EMD", "CI_Lower_EMD", "CI_Upper_EMD"])
        writer.writerow([NAME_EXPERIMENT, N_SIMULATION, type, MAE, MAE_CI_lower,
                        MAE_CI_higher, EMD_normalize, EMD_CI_lower, EMD_CI_higher])
...
def setup(env: simpy.Environment, NAME_EXPERIMENT, params, i, type):
    ...
    if type == 'rims':
        path_result = 'RIMS/results/simulated_log_LSTM_' + \
            NAME_EXPERIMENT + str(i) + '.csv'
    elif type == 'DDPS':
        path_result = 'DDPS_models_data/results/simulated_log_LSTM_' + \
            NAME_EXPERIMENT + str(i) + '.csv'
    elif type == 'DSIM':
        path_result = 'DSIM/results/simulated_log_LSTM_' + \
            NAME_EXPERIMENT + str(i) + '.csv'
    elif type == 'LSTM':
        path_result = 'LSTM_model_data/results/simulated_log_LSTM_' + \
            NAME_EXPERIMENT + str(i) + '.csv'
    else:
        path_result = 'RIMS/results/simulated_log_LSTM_' + \
            NAME_EXPERIMENT + str(i) + '.csv'
    ...
    for i in range(1, len(params.ARRIVALS) + 1):
        ...
        if type == 'DDPS' or type == 'LSTM':
            if type == 'LSTM' and NAME_EXPERIMENT in ["SynLoan", "ConsultaDataMining201618", "PurchasingExample", "BPI_Challenge_2017_W_Two_TS", "Productions"]:
                next = datetime.strptime(
                    params.ARRIVALS.loc[i - 1].at["start_timestamp"], '%Y-%m-%d %H:%M:%S')
            else:
                next = datetime.strptime(
                    params.ARRIVALS.loc[i - 1].at["start_timestamp"], '%Y-%m-%d %H:%M:%S.%f')

        if type == 'DSIM':
            next = datetime.strptime(
                params.ARRIVALS.loc[i - 1].at["start:timestamp"], '%Y-%m-%d %H:%M:%S')
