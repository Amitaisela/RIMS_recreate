...
class Parameters(object):
    ...
    def __init__(self, name_exp, iterations, type1, event_log_df):
        ...
        RIMS_plus_with_A = ["BPI_Challenge_2012_W_Two_TS", "BPI_Challenge_2017_W_Two_TS", "confidential_1000",
                            "cvs_pharmacy","SynLoan"]
        RIMS_with_S = ["BPI_Challenge_2012_W_Two_TS","Productions","PurchasingExample"]
        RIMS_plus_with_S = []
        ...
        if self.type1 == 'rims_plus':
            self.PATH_PETRINET = 'RIMS/' + self.NAME_EXP + '/' + type1 + '/' + self.NAME_EXP + '.pnml'
            self.MODEL_PATH_PROCESSING = 'RIMS/' + self.NAME_EXP + '/' + type1 + '/' + self.NAME_EXP + self.prefix[1] + '.h5'
            self.MODEL_PATH_WAITING = 'RIMS/' + self.NAME_EXP + '/' + type1 + '/' + self.NAME_EXP + self.prefix[2] + '.h5'
        else:
            self.PATH_PETRINET = 'RIMS/' + self.NAME_EXP + '/rims/' + self.NAME_EXP + '.pnml'
            self.MODEL_PATH_PROCESSING = 'RIMS/' + self.NAME_EXP + '/rims/' + self.NAME_EXP + self.prefix[1] + '.h5'
            self.MODEL_PATH_WAITING = 'RIMS/' + self.NAME_EXP + '/rims/' + self.NAME_EXP + self.prefix[2] + '.h5'
        ...
        if self.type1 == 'DSIM':
            if self.NAME_EXP == "SynLoan":
                self.ARRIVALS = pd.read_csv('DSIM/' + self.NAME_EXP + '/gen_synthetic_log_2000_11_5_' + str(iterations + 1) + '.csv')
                self.START_SIMULATION = datetime.strptime(self.ARRIVALS.loc[0].at["start:timestamp"], '%Y-%m-%d %H:%M:%S')
            else:
                self.ARRIVALS = pd.read_csv('DSIM/' + self.NAME_EXP + '/gen_' + self.NAME_EXP + '_' + str(iterations + 1) + '.csv')
                self.START_SIMULATION = datetime.strptime(self.ARRIVALS.loc[0].at["start:timestamp"], '%Y-%m-%d %H:%M:%S')
            ...
            if self.NAME_EXP == "SynLoan":
                self.ARRIVALS = pd.read_csv('LSTM_model_data/' + self.NAME_EXP + '/gen_synthetic_log_2000_11_5_' + str(iterations + 1) + '.csv')
                self.START_SIMULATION = datetime.strptime(self.ARRIVALS.loc[0].at["start_timestamp"], '%Y-%m-%d %H:%M:%S')
            ...
            self.ARRIVALS = pd.read_csv( 'RIMS/' + self.NAME_EXP + '/arrivals/iarr' + str(iterations) + '.csv', sep=',')
            self.START_SIMULATION = datetime.strptime(self.ARRIVALS.loc[0].at["timestamp"], '%Y-%m-%d %H:%M:%S')

        self.N_TRACE = len(self.ARRIVALS)

        if self.type1 == 'rims_plus':
            self.METADATA = 'RIMS/' + self.NAME_EXP + '/' + type1 + '/' + self.NAME_EXP + self.prefix[0] + '_meta.json'
            self.SCALER = 'RIMS/' + self.NAME_EXP + '/' + type1 + '/' + self.NAME_EXP + self.prefix[0] + '_scaler.pkl'
            self.INTER_SCALER = 'RIMS/' + self.NAME_EXP + '/' + type1 + '/' + self.NAME_EXP + self.prefix[0] + '_inter_scaler.pkl'
            self.END_INTER_SCALER = 'RIMS/' + self.NAME_EXP + '/' + type1 + '/' + self.NAME_EXP + self.prefix[
                0] + '_end_inter_scaler.pkl'
        else:
            self.METADATA = 'RIMS/' + self.NAME_EXP + '/rims/' + self.NAME_EXP + self.prefix[0] + '_meta.json'
            self.SCALER = 'RIMS/' + self.NAME_EXP + '/rims/' + self.NAME_EXP + self.prefix[0] + '_scaler.pkl'
            self.INTER_SCALER = 'RIMS/' + self.NAME_EXP + '/rims/' + self.NAME_EXP + self.prefix[0] + '_inter_scaler.pkl'
    ...
    def get_predefined_waiting_time(self, cid, transition, time):
        ...
        if len(waiting_time_row) > 0:
            waiting_time = (waiting_time_row['time:timestamp'].values[0] - time).total_seconds()
            return waiting_time
