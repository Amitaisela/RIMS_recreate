...
class Predictor(object):
    ...
    def __init__(self, model_path, params):
        ...
        self.parms = {
            ...
            'event_logs_path': 'input_files/event_logs',
            'bpmn_models': 'input_files/bpmn_models',
            'embedded_path': 'input_files/embedded_matix',
            'ia_gen_path': 'input_files/ia_gen_models',
            'seq_flow_gen_path': 'input_files/seq_flow_gen_models',
            'times_gen_path': 'input_files/times_gen_models',
            'sm1_path': 'external_tools/splitminer/splitminer.jar',
            'sm2_path': 'external_tools/splitminer2/sm2.jar',
            'sm3_path': 'external_tools/splitminer3/bpmtk.jar',
            'bimp_path': 'external_tools/bimp/qbp-simulator-engine.jar',
            'align_path': 'external_tools/proconformance/ProConformance2.jar',
            'calender_path': 'external_tools/calenderimp/CalenderImp.jar',
            'simulator': 'bimp',
            'sim_metric': 'tsd',
    ...
    def predict(self):
        if os.path.exists(self.params.METADATA):
            with open(self.params.METADATA) as file:
                ...
                pr_act_initial = int(round(data['inter_mean_states']['wip']))
    ...
    def post_simulation_adjustment(self, simulated_traces):
        """
        This method is specifically for the DSIM model.
        After the simulation is completed, it adjusts the waiting and processing times based on predictions.
        """
        for trace in simulated_traces:
            for event in trace:
                # Predict and adjust waiting time for DSIM post-simulation
                event['predicted_wait_time'] = self.predict_waiting_queue(event['caseid'], event['wip'],
                                                                          event['transition'], event['rp_oc'],
                                                                          event['time'], event['queue'])

                # Predict and adjust processing time for DSIM post-simulation
                event['predicted_process_time'] = self.processing_time(event['caseid'], event['wip'],
                                                                       event['transition'], event['ac_wip'],
                                                                       event['rp_oc'], event['time'], event['queue'])
    ...
    def processing_time(self, cid, pr_wip, transition, ac_wip, rp_oc, time, queue):
        if cid not in self.active_instances:
            self.active_instances[cid] = en.ProcessInstance(cid, self.n_size, (self.n_feat_proc, self.n_feat_wait),
        ...
        with warnings.catch_warnings():
            ...
            with self.g1.as_default():
                with self.first_session.as_default():
                    preds = self.proc_model_path.predict({'ac_input': np.array([act_ngram]), 'features': feat_ngram})
        ...
        with warnings.catch_warnings():
            ...
            with self.g2.as_default():
                with self.second_session.as_default():
                    preds = self.wait_model_path.predict({'ac_input': np.array([n_act_ngram]), 'features': feat_ngram})
        preds[preds < 0] = 0.000001
        wait_t = preds[0]
    ...
    def predict_waiting(self, cid, pr_wip, next_act, rp_oc, time, queue):
        """
        Predict waiting time for an instance without considering a queue.
        """
        ...
        # Prepare feature inputs for the waiting time prediction model
        wip = self.end_inter_scaler.transform(np.array([pr_wip]).reshape(-1, 1))[0]
        self.active_instances[cid].update_wait_ngram(next_act[0], time, wip, rp_oc)
        n_act_ngram, feat_ngram = self.active_instances[cid].get_wait_ngram()

        # Predict using the waiting time model
        with warnings.catch_warnings():
            warnings.filterwarnings('ignore')
            with self.g2.as_default():
                with self.second_session.as_default():
        ...
        # Update instance with predicted wait time
        self.active_instances[cid].update_wait(wait_t)

        # Scale back the predicted wait time to the original scale
        ipred = self.scaler.inverse_transform(np.concatenate((np.array([[0.000]]), preds), axis=1))
        iwait_t = ipred[0][1]
    ...
    @staticmethod
    def _encode_sequences(sequences, ac_idx, rl_task, rl_table):
        seq = sequences.copy()
